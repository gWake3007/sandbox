...
//!============================= Example (Search for words with the first letters of the alphabet) ============================

const langs = ["python", "javascript", "c++", "haskel", "php", "ruby"];

console.log("Sort: ", langs.sort());         //Сортування методом сорт!!

for(let i = 0; i < langs.length; i += 1){
    for(let k = i + 1; k < langs.length; k += 1){
        console.log(i, k);                   // Принцип праці скрипта(як перебирає)
        if(langs[i] > langs[k]){
            const temp = langs[i];            //Створюється для того щоб не втратити значення елемента
            langs[i] = langs[k];          // ПРИКЛАД ЗАМІНИ ЗНАЧЕННЯ МАСИВУ!!! ЗАПАМ'ЯТАТИ!!!
            langs[k] = temp;               //Обмін властивостями елементів якщо виконується умова(для того і була створенна змінна temp)
        }
    }
}

console.log("Our: ", langs); // Наш подвійний цикл для перебирання массиву (і виставляння елементів массиву по алфавіту першої літери)

// Скрипт порівнює одразу python з javascript і далі з усіма іншими елементами массиву.Тому i = 0 a k = i + 1;(тобто 1)
// [i] бере перший елемент массиву тобто з індексом 0 а [k] бере другий елемента массиву тобто з індексом 1 і так далі
